...
状态：架构与第一版范围确定，Herdr 接口、手机网络及硬件仍待实测。未决产品语义见 decisions.md。
...
正常状态聚合：blocked > working > done > idle。多个 Agent 中仍有工作者时保持蓝色；有阻塞者时红色。正常等待下一条新任务不能直接认定为 blocked。

绿灯不设 10 秒之类的完成计时器；新工作、阻塞或通信故障可以改变显示。已完成会话关闭后清除该会话的完成状态，按剩余有效会话重新聚合；若无其他有效任务则 idle 熄灯（D-004，用户已确认）。采集失败或心跳超时仍显示 unknown，不因缓存为空误判空闲。部分会话 unknown 的处理也需在状态采样后明确。
...
- 正式协议开发时明确版本、来源、发送进程实例标识、序列号和状态；序列号重启行为必须定义。接收端以本地经过时间检测超时，不依赖两端时钟严格同步。
- 最小手动 Demo 包为 POST /state，JSON 如 {"state":"blocked"}。正式同步包另行写入协议文档并测试。

开发仅监听 127.0.0.1:8787；远程联调显式绑定接收端具体 Tailscale IP。访问限制依靠私网范围和实际访问策略验证，不把“安装了 Tailscale”当作访问控制已经完成。

## 演示与故障边界

提供显式 Demo 模式，可手动切换所有 preset；Demo 期间真实心跳不覆盖演示状态，退出后恢复最新有效状态，过期则 unknown。
